...
func test_property_normalization_negative_angles():
	...
	for i in range(NUM_ITERATIONS):
		...
		assert_true(
			normalized >= 0.0,
			(
				"Normalized heading should be >= 0 (iteration %d: input=%.2f, output=%.2f)"
				% [i, negative_angle, normalized]
			)
		)
		assert_true(
			normalized < 360.0,
			(
				"Normalized heading should be < 360 (iteration %d: input=%.2f, output=%.2f)"
				% [i, negative_angle, normalized]
			)
		)
...
func test_property_forward_direction_normalized():
	...
	for i in range(NUM_ITERATIONS):
		...
		assert_almost_eq(
			...
			1.0,
			0.01,
			"Forward direction should be normalized (iteration %d: length=%.4f)" % [i, length]
		)
...
func test_property_heading_error_shortest_path():
	...
	for i in range(NUM_ITERATIONS):
		...
		assert_true(
			error >= -180.0, "Heading error should be >= -180 (iteration %d: %.2f)" % [i, error]
		)
		assert_true(
			error <= 180.0, "Heading error should be <= 180 (iteration %d: %.2f)" % [i, error]
		)
...
func test_property_heading_to_vector2_normalized():
	...
	for i in range(NUM_ITERATIONS):
		...
		assert_almost_eq(
			...
			0.01,
			(
				"Vector2 should be normalized (iteration %d: heading=%.2f, length=%.4f)"
				% [i, heading, length]
			)
		)
...
func test_property_round_trip_consistency():
	...
	for i in range(NUM_ITERATIONS):
		...
		assert_true(
			diff < 0.5,
			(
				"Round-trip heading should match (iteration %d: original=%.2f, recovered=%.2f, diff=%.2f)"
				% [i, original_heading, recovered_heading, diff]
			)
		)
...
func test_property_forward_direction_heading_consistency():
	...
	for i in range(NUM_ITERATIONS):
		...
		assert_true(
			diff < 0.1,
			(
				"Heading should match inverted yaw rotation (iteration %d: yaw=%.2f, heading=%.2f, expected=%.2f, diff=%.2f)"
				% [i, yaw_degrees, heading, expected_heading, diff]
			)
		)
