...
func test_dive_to_50m_stability() -> void:
	...
	# Assertions
	assert_almost_eq(analysis.final_depth, 50.0, 3.0,
		"Final depth should be within 3m of target")
	
	assert_lt(analysis.overshoot, 10.0,
		"Overshoot should be less than 10m")
	
	assert_lt(analysis.oscillation_count, 8,
		"Should have fewer than 8 oscillations (indicates instability)")
	
	assert_lt(analysis.settling_time, 60.0,
		"Should settle within 60 seconds")
...
func test_surface_to_100m_dive() -> void:
	...
	assert_almost_eq(analysis.final_depth, 100.0, 5.0,
		"Final depth should be within 5m of target")
...
func test_depth_change_mid_dive() -> void:
	...
	assert_almost_eq(final_depth, 70.0, 5.0,
		"Should reach new target depth after change")
