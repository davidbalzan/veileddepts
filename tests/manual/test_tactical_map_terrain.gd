...
func _init():
	...
	if terrain_renderer.initialized:
		...
		print(
			"  Heightmap size: ",
			terrain_renderer.heightmap.get_width(),
			"x",
			terrain_renderer.heightmap.get_height()
		)
	...
	if tactical_map.terrain_texture:
		...
		print(
			"  Texture size: ",
			tactical_map.terrain_image.get_width(),
			"x",
			tactical_map.terrain_image.get_height()
		)
	else:
	...
	if tactical_map.terrain_image:
		var sample_positions = [
			Vector2i(64, 64), Vector2i(128, 128), Vector2i(192, 192), Vector2i(32, 200)
		]

		print("Sampling terrain colors (should show variation):")
		for pos in sample_positions:
			if (
				pos.x < tactical_map.terrain_image.get_width()
				and pos.y < tactical_map.terrain_image.get_height()
			):
				...
				print(
					(
						"  Position %s: RGB(%.2f, %.2f, %.2f) - %s"
						% [pos, color.r, color.g, color.b, color_desc]
					)
				)

	# Test 4: Verify world-to-screen conversion with terrain
	print("\n--- Test 4: Coordinate Conversion ---")
	var test_world_positions = [Vector3(0, 0, 0), Vector3(500, 0, 500), Vector3(-500, 0, -500)]
	...
	for world_pos in test_world_positions:
		...
		print(
			(
				"  World %s → Screen %s → World %s (error: %.2f)"
				% [world_pos, screen_pos, back_to_world, error]
			)
		)
