...
func test_ai_patrol_navigation_and_detection():
	...
	gut.p("✓ AI patrol detected by submarine at range: %.1fm" % contact.range)
	...
	gut.p("✓ AI patrol navigating waypoints (current: %d/%d)" % [agent.current_waypoint_index, agent.patrol_waypoints.size()])
...
func test_sonar_detection_and_tracking():
	...
	var contacts_data = [
		...
		{"type": Contact.ContactType.SURFACE_SHIP, "pos": Vector3(5000, 0, 0), "name": "Surface Ship"},
	...
	assert_true(aircraft_contact.detected, "Aircraft should be detected by radar at periscope depth")
...
func test_full_simulation_integration():
	...
	# Update sonar to detect contact
	sonar_system._update_passive_sonar()
	
	# Verify contact detection
	contact = simulation_state.get_contact(agent.contact_id)
	assert_true(contact.detected, "Contact should be detected by sonar")
	gut.p("✓ Sonar detected AI patrol")
	...
	gut.p("✓ Submarine physics updating (depth: %.1fm, speed: %.1fm/s)" % [sub_state["depth"], sub_state["speed"]])
	...
	gut.p("✓ Terrain preventing submarine penetration (sub: %.1fm, terrain: %.1fm)" % [sub_y, terrain_height])
