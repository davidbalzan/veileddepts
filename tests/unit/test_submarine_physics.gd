...
func test_depth_clamping_at_surface():
	...
	assert_lte(
		submarine_body.global_position.y, 2.0, "Submarine should not go too far above surface"
	)
...
func test_physics_integration_with_simulation_state():
	...
	assert_eq(
		simulation_state.submarine_position, physics_state["position"], "Position should sync"
	)
	assert_eq(
		simulation_state.submarine_velocity, physics_state["velocity"], "Velocity should sync"
	)
	assert_almost_eq(
		simulation_state.submarine_depth, physics_state["depth"], 0.1, "Depth should sync"
	)
	assert_almost_eq(
		simulation_state.submarine_speed, physics_state["speed"], 0.1, "Speed should sync"
	)
...
func test_physics_summary():
	...
	simulation_state.update_submarine_command(Vector3(1000, 0, 1000), 8.0, 75.0)
