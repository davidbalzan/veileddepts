...
func test_debug_mode_propagates_to_rudder_system():
	...
	assert_false(physics_v2.rudder_system.debug_mode, "Rudder system should have debug mode disabled")

func test_log_callback_is_set():
	# Requirement 14.1: log callback should be set on components
	physics_v2.debug_mode = true
	assert_true(physics_v2.rudder_system.log_callback.is_valid(), "Rudder system should have valid log callback")
...
func test_velocity_clamping_logging():
	...
	assert_lte(final_speed, physics_v2.max_speed * 1.1, "Velocity should be clamped to 110% of max speed")
...
func test_boundary_hit_logging():
	...
	assert_lte(submarine_body.global_position.x, physics_v2.map_boundary, "Position should be clamped to boundary")
