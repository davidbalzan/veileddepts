...
func test_zoom_clamping():
	...
	assert_gte(tactical_map_view.map_zoom, tactical_map_view.MIN_ZOOM, "Zoom should not go below min")
...
func test_ui_elements_created():
	...
	assert_not_null(tactical_map_view.submarine_info_label, "Submarine info label should be created")
...
func test_speed_slider_range():
	...
	assert_almost_eq(tactical_map_view.speed_slider.max_value, SimulationState.MAX_SPEED, 0.01, "Speed max should match simulation")


## Test depth slider range matches simulation limits
func test_depth_slider_range():
	assert_almost_eq(tactical_map_view.depth_slider.min_value, SimulationState.MIN_DEPTH, 0.01, "Depth min should match simulation")
	assert_almost_eq(tactical_map_view.depth_slider.max_value, SimulationState.MAX_DEPTH, 0.01, "Depth max should match simulation")
