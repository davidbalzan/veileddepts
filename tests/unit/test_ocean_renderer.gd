...
func test_wind_direction_property():
	...
	assert_almost_eq(
		ocean_renderer.wind_direction_degrees, 90.0, 0.001, "Wind direction should be set"
	)
...
func test_set_wind():
	...
	assert_almost_eq(
		ocean_renderer.wind_direction_degrees, 180.0, 0.001, "Wind direction should be updated"
	)
...
func test_fft_resolution_index():
	assert_eq(
		ocean_renderer.fft_resolution_index, 2, "Default FFT resolution index should be 2 (256x256)"
	)


func test_horizontal_dimension():
	assert_eq(
		ocean_renderer.horizontal_dimension, 512, "Default horizontal dimension should be 512"
	)
