...
func test_periscope_rotation_input_updates_view():
	...
	assert_eq(
		periscope_view.periscope_rotation,
...
func test_periscope_zoom_input_updates_fov():
	...
	# FOV should decrease
	assert_eq(
		periscope_view.zoom_level, initial_fov - 10.0, "Periscope zoom should respond to input"
	)

	# Camera FOV should match
	assert_eq(
		periscope_view.camera.fov, periscope_view.zoom_level, "Camera FOV should match zoom level"
	)
...
func test_periscope_zoom_clamped_to_limits():
	...
	assert_eq(
		periscope_view.zoom_level, PeriscopeView.MIN_FOV, "Zoom should be clamped to minimum FOV"
	)
	...
	assert_eq(
		periscope_view.zoom_level, PeriscopeView.MAX_FOV, "Zoom should be clamped to maximum FOV"
	)
...
func test_periscope_underwater_mode_activation():
	...
	assert_not_null(
		periscope_view.underwater_environment, "Underwater environment should be created"
	)
	...
	assert_false(periscope_view.is_underwater_mode, "Should not be in underwater mode at surface")
	...
	assert_not_null(
		periscope_view.camera.environment,
		"Camera should have underwater environment when submerged"
	)
	assert_true(periscope_view.is_underwater_mode, "Underwater mode flag should be set")
...
func test_periscope_pitch_input():
	...
	# Pitch should change
	assert_eq(
		periscope_view.periscope_pitch,
		initial_pitch + 20.0,
...
func test_external_camera_tilt_input():
	...
	# Tilt should change
	assert_eq(external_view.camera_tilt, initial_tilt + 15.0, "Camera tilt should respond to input")
...
func test_external_camera_tilt_clamped():
	...
	assert_eq(external_view.camera_tilt, ExternalView.MIN_TILT, "Tilt should be clamped to minimum")

	# Try to tilt beyond maximum
	external_view.camera_tilt = 85.0
	external_view.handle_tilt_input(10.0)

	assert_eq(external_view.camera_tilt, ExternalView.MAX_TILT, "Tilt should be clamped to maximum")
...
func test_fog_of_war_system_filters_correctly():
	...
	assert_true(
		fog_of_war.is_contact_visible(contact_visible), "Visible contact should pass fog of war"
	)
	assert_false(
		fog_of_war.is_contact_visible(contact_hidden), "Hidden contact should fail fog of war"
	)
...
func test_full_integration_commands_flow_through_physics():
	...
	# Issue commands
	simulation_state.update_submarine_command(Vector3(500, 0, 500), 7.5, 100.0)  # Waypoint  # Speed  # Depth
...
func test_full_integration_contact_visibility_across_views():
	...
	assert_eq(
		visible_from_external.size(),
		visible_from_sim.size(),
