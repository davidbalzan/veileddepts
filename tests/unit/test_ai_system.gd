...
func test_spawn_air_patrol():
	...
	var patrol_route: Array[Vector3] = [
		Vector3(100, 200, 100), Vector3(200, 200, 200), Vector3(100, 200, 300)
	]
...
func test_spawn_circular_patrol():
	...
	assert_eq(ai_system.get_agent_count(), 1, "AISystem should have 1 agent")
	assert_eq(
		agent.patrol_waypoints.size(),
		num_waypoints,
		"Agent should have correct number of waypoints"
	)
...
func test_contact_detection_updates():
	...
	assert_false(
		contact.identified, "Contact should not be identified at 3km (beyond visual range)"
	)
