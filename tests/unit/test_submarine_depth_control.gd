...
class MockOceanRenderer:
	extends OceanRenderer

	func _ready() -> void:
		# Override to avoid real setup
		pass

	func get_wave_height_3d(_pos: Vector3) -> float:
...
func test_depth_control_reaches_target() -> void:
	...
	while time_elapsed < TEST_DURATION:
		...
		simulation_state.update_submarine_state(
			state.position, state.velocity, state.depth, state.heading, state.speed
		)
	...
	assert_almost_eq(
		...
		STEADY_STATE_ERROR,
		(
			"Final depth should be within %fm of target. Got: %.2fm, Expected: %.2fm"
			% [STEADY_STATE_ERROR, final_depth, TARGET_DEPTH]
		)
	)

	assert_lt(
		max_depth_reached,
		TARGET_DEPTH + ACCEPTABLE_OVERSHOOT,
		(
			"Maximum depth should not overshoot target by more than %fm. Max: %.2fm, Target: %.2fm"
			% [ACCEPTABLE_OVERSHOOT, max_depth_reached, TARGET_DEPTH]
		)
	)
...
func test_depth_control_no_oscillation() -> void:
	...
	for i in range(settling_index, depth_history.size()):
		...
		if (
			i > settling_index
			and sign(current_error) != sign(previous_error)
			and abs(current_error) > 1.0
		):
			oscillation_count += 1
	...
	assert_lt(
		...
		5,
		(
			"Should have minimal oscillations after settling. Found %d zero crossings"
			% oscillation_count
		)
	)
...
func test_pid_controller_gains() -> void:
	...
	assert_gt(
		rise_time, 5.0, "Rise time should be > 5s for realistic behavior. Got: %.2fs" % rise_time
	)
	assert_lt(
		rise_time, 25.0, "Rise time should be < 25s for responsive control. Got: %.2fs" % rise_time
	)
...
func test_depth_control_with_speed() -> void:
	...
	assert_almost_eq(
		...
		(
			"Should reach target depth while moving. Got: %.2fm, Expected: %.2fm"
