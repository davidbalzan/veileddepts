...
func test_world_to_chunk_positive_coordinates():
	...
	# Test origin
	assert_eq(
		coords.world_to_chunk(Vector3(0, 0, 0)), Vector2i(0, 0), "Origin should be chunk (0,0)"
	)
...
func test_world_to_chunk_negative_coordinates():
	...
	assert_eq(
		coords.world_to_chunk(Vector3(-1, 0, -1)),
		Vector2i(-1, -1),
		"Negative position should map to negative chunk"
	)
	assert_eq(
		coords.world_to_chunk(Vector3(-256, 0, -256)), Vector2i(-1, -1), "Center of chunk (-1,-1)"
	)
	assert_eq(
		coords.world_to_chunk(Vector3(-512, 0, -512)), Vector2i(-1, -1), "Edge of chunk (-1,-1)"
	)
	assert_eq(
		coords.world_to_chunk(Vector3(-513, 0, -513)), Vector2i(-2, -2), "Start of chunk (-2,-2)"
	)
...
func test_world_to_chunk_boundaries():
	...
	assert_eq(
		coords.world_to_chunk(Vector3(511.9, 0, 511.9)), Vector2i(0, 0), "Just before boundary"
	)
	assert_eq(
		coords.world_to_chunk(Vector3(512.0, 0, 512.0)), Vector2i(1, 1), "Exactly at boundary"
	)
	assert_eq(
		coords.world_to_chunk(Vector3(512.1, 0, 512.1)), Vector2i(1, 1), "Just after boundary"
	)
...
func test_chunk_to_world_center():
	...
	assert_eq(
		coords.chunk_to_world(Vector2i(-1, -1)), Vector3(-256, 0, -256), "Center of chunk (-1,-1)"
	)
...
func test_get_chunk_corner():
	...
	assert_eq(
		coords.get_chunk_corner(Vector2i(1, 1)), Vector3(512, 0, 512), "Corner of chunk (1,1)"
	)
	assert_eq(
		coords.get_chunk_corner(Vector2i(-1, -1)), Vector3(-512, 0, -512), "Corner of chunk (-1,-1)"
	)
...
func test_is_position_in_chunk():
	...
	assert_true(
		coords.is_position_in_chunk(Vector3(0, 0, 0), Vector2i(0, 0)), "Origin in chunk (0,0)"
	)
	assert_true(
		coords.is_position_in_chunk(Vector3(256, 0, 256), Vector2i(0, 0)), "Center in chunk (0,0)"
	)
	assert_true(
		coords.is_position_in_chunk(Vector3(511, 0, 511), Vector2i(0, 0)),
		"Near edge in chunk (0,0)"
	)

	# Test positions outside chunk (0,0)
	assert_false(
		coords.is_position_in_chunk(Vector3(512, 0, 512), Vector2i(0, 0)), "Outside chunk (0,0)"
	)
	assert_false(
		coords.is_position_in_chunk(Vector3(-1, 0, -1), Vector2i(0, 0)),
		"Negative outside chunk (0,0)"
	)
