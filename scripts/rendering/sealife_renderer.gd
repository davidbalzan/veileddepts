...
func _setup_sealife() -> void:
	...
	if rd == null:
		push_warning(
			"SealifeRenderer: No RenderingDevice available (headless mode?). Using fallback."
		)
		_initialized = true
		return
...
func _initialize_fish_school() -> void:
	...
	for i in range(fish_count):
		...
		var pos = Vector3(cos(angle) * radius, depth, sin(angle) * radius)
		...
		var vel = Vector3(cos(swim_angle) * speed, 0, sin(swim_angle) * speed)
...
func _animate_fish(delta: float) -> void:
	...
	var count = mini(
		mini(_fish_positions.size(), _fish_velocities.size()),
		mini(_fish_scales.size(), _fish_wave_offsets.size())
	)
	if count == 0:
		return
	...
	for i in range(count):
		...
		var wave_motion = (
			sin(Time.get_ticks_msec() * 0.001 * wave_frequency + wave_offset) * wave_amplitude
		)
