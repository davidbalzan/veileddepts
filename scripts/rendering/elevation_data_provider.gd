...
func initialize() -> bool:
	...
	if _load_elevation_map():
		...
		print("  Min elevation: %.1f m (pixel value: %.3f)" % [MARIANA_TRENCH_DEPTH, _min_pixel_value])
		print("  Max elevation: %.1f m (pixel value: %.3f)" % [MOUNT_EVEREST_HEIGHT, _max_pixel_value])
		return true
	else:
