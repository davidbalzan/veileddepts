...
var parameters = {
	"buoyancy_point_offset": [-5.0, 10.0, 0.5, 2.0],  # How low sub sits in water (top of list for easy access)
...
func _ready() -> void:
	...
	DebugPanelManager.register_panel("submarine", self)
...
func _create_ui() -> void:
	...
	for param_name in parameters.keys():
		...
		_create_parameter_slider(
			vbox, param_name, param_data[0], param_data[1], param_data[2], param_data[3]
		)
