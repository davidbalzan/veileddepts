...
func _init(
	...
	assert(
		sideways_drag >= base_forward_drag * 400.0,
...
func calculate_drag_force(
	velocity: Vector3, forward_dir: Vector3, depth: float, appendage_registry
) -> Vector3:
	...
	var forward_drag_magnitude: float = (
		total_forward_drag_coef * forward_speed_abs * forward_speed_abs
	)
