...
func validate_and_fix_submarine_state(body: RigidBody3D) -> bool:
	...
	if not body.global_position.is_finite():
		_log_error_throttled(
			"position_nan", "NaN detected in submarine position, resetting to origin"
		)
	...
	if not body.linear_velocity.is_finite():
		_log_error_throttled(
			"velocity_nan", "NaN detected in submarine velocity, resetting to zero"
		)
	...
	if not body.angular_velocity.is_finite():
		_log_error_throttled(
			"angular_velocity_nan", "NaN detected in submarine angular velocity, resetting to zero"
		)
...
func enforce_boundaries(body: RigidBody3D, boundary: float) -> void:
	...
	if hit_boundary:
		_log_error_throttled(
			"boundary_hit",
			"Submarine hit map boundary at position: (%.1f, %.1f)" % [position.x, position.z]
		)
