...
func _create_debug_panel() -> void:
	...
	sea_level_value.text = "%.3f (%.0fm elevation, Default: 0.562)" % [current_normalized, current_meters]
	...
	# Sea level slider - focused range around 0.562 default
...
func _on_sea_level_changed(normalized: float, meters: float = NAN) -> void:
	...
	if _debug_panel:
		...
		if value_label:
			value_label.text = "%.3f (%.0fm elevation, Default: 0.562)" % [normalized, meters]
...
func _on_reset_sea_level() -> void:
	...
	if _debug_panel:
		var value_label = _debug_panel.find_child("SeaLevelValue", true, false)
		if value_label:
			value_label.text = "%.3f (%.0fm elevation, Default: 0.562)" % [SeaLevelManager.get_sea_level_normalized(), elevation_meters]
	...
	print("WholeMapView: Sea level reset to default (0.562 / 0m elevation)")
