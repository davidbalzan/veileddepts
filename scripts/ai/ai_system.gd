...
func spawn_air_patrol(
	spawn_position: Vector3, patrol_route: Array[Vector3], loop: bool = true
) -> AIAgent:
	# Create new AI agent
		...
		if simulation_state and agent.contact_id >= 0:
			...
			if contact:
				...
				if simulation_state.submarine_position:
					var distance = agent.global_position.distance_to(
...
func spawn_circular_patrol(
	center: Vector3, radius: float, num_waypoints: int = 4, altitude: float = 200.0
) -> AIAgent:
...
func spawn_linear_patrol(
	start_point: Vector3, end_point: Vector3, altitude: float = 200.0
) -> AIAgent:
	var patrol_route: Array[Vector3] = []
...
func spawn_grid_patrol(
	center: Vector3, grid_size: Vector2, spacing: float, altitude: float = 200.0
) -> Array[AIAgent]:
