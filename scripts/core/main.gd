...
func _create_submarine_body() -> void:
	...
	# Register submarine with terrain renderer for streaming
	if terrain_renderer:
		if terrain_renderer.has_method("set_submarine"):
			terrain_renderer.set_submarine(submarine_body)
		else:
			push_warning("Main: terrain_renderer does not have set_submarine method")
...
func _setup_terrain_renderer() -> void:
	...
	if terrain_renderer:
		...
		if terrain_renderer.initialized:
			...
			print("  Chunk-based streaming: enabled")
			print("  Load distance: %.0f m" % terrain_renderer.load_distance)
		else:
...
func _physics_process(delta: float) -> void:
	...
	if submarine_physics and submarine_body:
		...
		if (
			...
			in [ViewManager.ViewType.PERISCOPE, ViewManager.ViewType.EXTERNAL]
		):
...
func _manage_3d_visibility() -> void:
	...
	var is_2d_view = (
		view_manager.current_view
		in [ViewManager.ViewType.TACTICAL_MAP, ViewManager.ViewType.WHOLE_MAP]
	)
...
func teleport_submarine(target_position: Vector3) -> void:
	...
	if simulation_state:
		simulation_state.update_submarine_state(
			final_pos, Vector3.ZERO, -final_pos.y, rad_to_deg(current_heading), 0.0
		)

		# Also reset target waypoint and speed to prevent immediate movement
		simulation_state.update_submarine_command(final_pos, 0.0, -final_pos.y)
...
func teleport_and_shift(click_uv: Vector2) -> void:
	...
	var new_region = Rect2(click_uv.x - 0.05, click_uv.y - 0.05, 0.1, 0.1)
