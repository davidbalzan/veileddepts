...
func switch_to_view(view: ViewType) -> void:
	...
	# Log view switch to console
	if LogRouter:
		LogRouter.log(
			"Switching view: %s -> %s" % [_view_type_to_string(current_view), _view_type_to_string(view)],
	...
	# Log transition time to console
	if LogRouter:
		LogRouter.log(
			"View switch completed: %s (%.1fms)" % [_view_type_to_string(view), transition_time],
	...
	if transition_time > 100:
		push_warning(
			"ViewManager: View transition took %d ms (exceeds 100ms requirement)" % transition_time
		)
		
		# Log warning to console
		if LogRouter:
			LogRouter.log(
				"View transition exceeded 100ms requirement: %.1fms" % transition_time,
