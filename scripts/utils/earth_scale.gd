...
static func get_region_size_meters(region: Rect2) -> Vector2:
	return Vector2(region.size.x * FULL_MAP_WIDTH_METERS, region.size.y * FULL_MAP_HEIGHT_METERS)
...
static func get_region_for_size_meters(center_uv: Vector2, size_meters: Vector2) -> Rect2:
	...
	return Rect2(center_uv.x - uv_width / 2.0, center_uv.y - uv_height / 2.0, uv_width, uv_height)
...
static func get_uv_size_for_terrain(terrain_size_meters: float) -> Vector2:
	return Vector2(
		terrain_size_meters / FULL_MAP_WIDTH_METERS, terrain_size_meters / FULL_MAP_HEIGHT_METERS
	)
...
static func print_scale_info(region: Rect2) -> void:
	...
	print(
		(
			"UV Region: position=(%.4f, %.4f) size=(%.4f, %.4f)"
	...
	print(
		(
			"Full map: %.0f km x %.0f km"
			% [FULL_MAP_WIDTH_METERS / 1000.0, FULL_MAP_HEIGHT_METERS / 1000.0]
		)
	)
