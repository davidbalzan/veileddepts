...
func _init():
	...
	if sub_screen.distance_to(tactical_map.map_center) < 1.0:
		...
		print(
			"✗ Submarine not at center, distance: ", sub_screen.distance_to(tactical_map.map_center)
		)
	...
	if (
		abs(actual_offset.x - expected_offset) < 1.0
		and abs(actual_offset.y - expected_offset) < 1.0
	):
